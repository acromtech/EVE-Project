...
{
    FILE *baseDescripteur = fopen("../IndexationImage/IndexationRechercheImage/FichierIndexation/base_descripteur_image.csv", "a");
    FILE *listeDescripteur = fopen("../IndexationImage/IndexationRechercheImage/FichierIndexation/liste_descripteur_image.csv", "a");
    int nbComposantes;
    int histogramSize =1; 
...
void indexerBaseImage(baseDescripteurImage *bd, listeDescripteurImage *listeDescripteur)
{
    ...
    sprintf(command, "ls ../IndexationImage/TXT/*.txt > files.txt");
    system(command);
