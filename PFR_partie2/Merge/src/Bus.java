...
public class Bus {
    ...
    private Map<String, Double> mapTotalScore = new HashMap<>();
    ...
    protected void addMotor(Motor motor) {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    ProcessBuilder pb2 = new ProcessBuilder("./" + motor.getMakeTarget(), "" + MASTER_ID, String.valueOf(motor.getId()));
    ...
    protected void launchIndexation(Motor motor, final int addrTypeTraitement) {
        if (addrTypeTraitement == ADDR_INDEXATION_ALL || addrTypeTraitement == ADDR_INDEXATION_TEXTE || addrTypeTraitement == ADDR_INDEXATION_IMAGE || addrTypeTraitement == ADDR_INDEXATION_SON) {
            if (motor.getConnection()) sendFrame(motor, addrTypeTraitement, "" + 0x00, 0x00);
                ...
                System.out.println("JAVA\tErreur : Impossible d'envoyer la requete car le moteur " + motor.getId() + " est déconnecté");
        } else
            System.out.println("JAVA\tErreur : Le traitement que vous tenter de lancer n'est pas un traitement d'indexation");
    }
    ...
    private void sendFrame(Motor motor, final int addrTypeTraitement, String data1, int data2) {
        ...
        try {
            ...
            synchronized (bus) {
                while (!motor.getAck()) bus.wait();
            }
            if (affConsole) System.out.println("JAVA\tACK");
            synchronized (bus) {
                while (!motor.getEndProcessing()) bus.wait();
            }
    ...
    protected List<ScorePath> sendWord(Motor motor, final int addrTypeTraitement, String requete, int nombreResultatMax) {
        ...
        if (addrTypeTraitement == ADDR_TEXTE_FICHIER || addrTypeTraitement == ADDR_TEXTE_MOTCLE || addrTypeTraitement == ADDR_IMAGE_FICHIER || addrTypeTraitement == ADDR_IMAGE_MOTCLE || addrTypeTraitement == ADDR_SON_FICHIER) {
            ...
            System.out.println("JAVA\tErreur : Le traitement que vous tentez de lancer n'est pas un traitement de recherche");
    ...
    public Map<String, Double> sendRechercheComplexe(Motor motor, int typeTraitement, String recherche, int nbResultMax) {
        ...
        Map<String, Double> sortedMap = Tools.trieMapOrdreDecroiss(mapTotalScore);
    ...
    protected void setOpenMode(Motor motor, boolean openMode) {
        sendFrame(motor, ADDR_OPEN_MODE, String.valueOf(openMode), 0x00);
