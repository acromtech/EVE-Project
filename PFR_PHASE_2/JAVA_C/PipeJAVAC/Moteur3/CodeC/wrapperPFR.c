...
void sendAllResBus(ResListToSend* resListToSend){
	...
	while(1){
		if(autorisation_envoie_trame){
			if(nbResSend<nombreResultatMax && resListToSend!=NULL){
				autorisation_envoie_trame=0;
				IvySendMsg("%d %d %d %d %s",masterId,motorId,SCORE_CHEMIN,resListToSend->score,resListToSend->chemin);
				resListToSend = resListToSend->next;
				nbResSend++;
			}
			else{
				traitementEffectue();
