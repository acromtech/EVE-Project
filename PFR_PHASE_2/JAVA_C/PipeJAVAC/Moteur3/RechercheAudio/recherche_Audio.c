...
float comparaisonRepertoire(char * fichierReference){
    ...
    DIR *d = opendir("../FichierIndexation/Son/BaseDescripteursAudios/"); 
    ...
    if (d) {
        ...
        while ((dir = readdir(d)) != NULL){
			...
			if(strlen(dir->d_name)>3) {
				...
				char chemin[strlen("../FichierIndexation/Son/BaseDescripteursAudios/")+strlen(dir->d_name)+1];
				char nomFichier[strlen(dir->d_name)+1];
				strcpy(chemin,"../FichierIndexation/Son/BaseDescripteursAudios/");
    ...
    if (d) {
        while ((dir = readdir(d)) != NULL){
			if(!strcmp(dir->d_name,souhait)){
				...
				char chemin[strlen("../FichierIndexation/Son/BaseDescripteursAudios/")+1+ strlen(souhait)];
				strcpy(chemin,"../FichierIndexation/Son/BaseDescripteursAudios/");
...
struct resultatAudio saisieRechercheAudio(char *requete){
	char* souhait = calloc(1000, sizeof(char));
	strcpy(souhait,requete);
	char *fichierAudio = "../../BaseFichier/Son/corpus_fi.wav";
	float resultat;
	struct resultatAudio resultatAudio;
	resultatAudio.path = calloc(100, sizeof(char));
	resultat = resultatRechercheAudio(souhait);
	if(resultat != -1 && resultat != -2){
		resultatAudio.score = (int)resultat;
		strcpy(resultatAudio.path, fichierAudio);
	}
	printf("resultat = %s %d\n", resultatAudio.path, resultatAudio.score);
